...
Reads cn/occupations.json and scores each occupation. Results are cached
incrementally to cn/scores.json so the script can be resumed if interrupted.

Usage:
    uv run python cn/score.py
    uv run python cn/score.py --model google/gemini-3-flash-preview
    uv run python cn/score.py --start 0 --end 10   # test on first 10
"""
...
Examples: construction laborer, farmer, cleaner.
...
{
  "exposure": <0-10>,
  "rationale": "<2-3 sentences in Chinese explaining the key factors>"
}\
"""


def score_occupation(client, occ, model):
    """Send one occupation to the LLM and parse the structured response."""
    prompt = f"""职业名称: {occ['title']}
行业分类: {occ['category']}
年薪中位数: ¥{occ.get('median_pay_annual', 'N/A'):,}
学历要求: {occ.get('education', 'N/A')}
就业人数: {occ.get('num_jobs', 'N/A'):,}"""
